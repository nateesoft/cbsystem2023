...
public class CbFeeTransactionControl extends BaseControl {
    ...
    public List<CbFeeTransactionBean> mappingBean(ResultSet rs) throws Exception {
        List<CbFeeTransactionBean> listBean = new ArrayList<>();
        while (rs.next()) {
    ...
    public List<CbFeeTransactionBean> listCbFeeTransaction() {
        try {
            String sql = "select * from cb_fee_transaction order by fee_date, fee_time";
    ...
    public CbFeeTransactionBean findOneByFeeCode(String fee_code) {
        try {
            String sql = "select * from cb_fee_transaction where fee_code='" + fee_code + "'";
            ResultSet rs = MySQLConnect.getResultSet(sql);
