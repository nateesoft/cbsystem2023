...
public class ConfigDialog extends BaseDialogSwing {
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
        ...
        bean.setCode(txtBranchCode.getText());
        bean.setName(txtBranchName.getText());
        bean.setAddressNo(txtBranchAt.getText());
        bean.setLocality(txtTambol.getText());
        bean.setSubProvince(txtAmphur.getText());
        bean.setProvince(txtProvince.getText());
        bean.setPost(txtPost.getText());
        bean.setTel_No(txtTel.getText());
    ...
    private void loadListGroup() {
        List<CbGroupBean> listGroup = getGroupControl().listCbGroup();
        DefaultTableModel model = (DefaultTableModel) tbGroup.getModel();
        TableUtil.clearModel(model);
        cbGroup.removeAllItems();

        for (CbGroupBean groupBean : listGroup) {
            model.addRow(new Object[]{
                groupBean.getGroupcode(),
                groupBean.getGroupname(),
                groupBean.getPermission()
            });

            cbGroup.addItem(groupBean.getGroupcode());
        }
    }

    private void loadListUser() {
        List<CbUserBean> userList = getUserControl().listUser();
        DefaultTableModel model = (DefaultTableModel) tbUser.getModel();
        TableUtil.clearModel(model);

        for (CbUserBean userBean : userList) {
            model.addRow(new Object[]{
