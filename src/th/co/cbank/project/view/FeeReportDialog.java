...
public class FeeReportDialog extends BaseDialogSwing {
    ...
    private void loadData() {
        ...
        int count = 0;
        for (CbFeeTransactionBean bean : listFee) {
            count++;
            model.addRow(new Object[]{
                count,
                bean.getFee_desc(),
                NumberFormat.showDouble2(bean.getFee_amount()),
                bean.getFee_branch(),
                bean.getFee_emp_code(),
                bean.getFee_cust_code(),
                DateFormat.getLocale_ddMMyyyy(bean.getFee_date()),
                bean.getFee_time()
            });
    ...
    private void exportReport() {
        ...
        Map m = new HashMap();
        m.put("sql", "select * from cb_fee_transaction order by fee_date, fee_time");
